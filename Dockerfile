...
RUN apt-get install --no-install-suggests -y ca-certificates php8.2 php8.2-fpm php8.2-common \
    php8.2-mysql php8.2-zip php8.2-gd php8.2-mbstring php8.2-curl php8.2-sqlite3 \
    php8.2-bcmath php8.2-xml php8.2-intl php8.2-tidy php-common php-imagick imagemagick composer
...
RUN echo "daemonize = no" > /etc/php/8.2/fpm/php-fpm.conf \
    && echo "include=/etc/php/8.2/fpm/pool.d/*.conf" >> /etc/php/8.2/fpm/php-fpm.conf
RUN echo "max_execution_time = 200" >> /etc/php/8.2/fpm/php.ini \
    && echo "post_max_size = 100M" >> /etc/php/8.2/fpm/php.ini \
    && echo "upload_max_filesize = 20M" >> /etc/php/8.2/fpm/php.ini \
